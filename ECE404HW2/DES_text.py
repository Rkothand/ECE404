...
def Decrypt(round_key_in):
    round_key_in = list(reversed(round_key_in))
    fp = open(sys.argv[4], 'w')

    fpd = open(sys.argv[2],'r')
    bitvec = BitVector(hexstring=fpd.read())
    fp = open(sys.argv[4], 'w')
    for i in range( len(bitvec) // 64):
        bitVec1 = bitvec[i * 64 : (i+1) *64]
        if len(bitVec1) > 0:
            if len(bitVec1) != 64:
        ...
        for roundkey in round_key_in:
            newRE = RE.permute( expansion_permutation )
            out_xor = newRE ^ roundkey
            sbox = substitute(out_xor)
            pbox = sbox.permute(permutation_box)
            p1 = pbox ^ LE 
            LE = RE
            RE = p1
        BitVectorout = RE+LE
        # print(BitVectorout.get_bitvector_in_ascii())
        fp.write(BitVectorout.get_bitvector_in_ascii())
        # BitVectorout.write_to_file(fp)
    fp.close()
...
def Encrypt_Decrypt():
    ...
    if(sys.argv[1]== "-d"):
        ...
        while (bitVec1.more_to_read):
            ...
            fp.write(BitVectorout.get_bitvector_in_hex())
        fp.close()
